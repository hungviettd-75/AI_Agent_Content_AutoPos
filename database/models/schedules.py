...
class ScheduleModel:
    ...
    @staticmethod
    def get_by_id(schedule_id: int) -> dict:
    ...
    def update_status(schedule_id: int, status: str,
                      error_message: str = None, published_at: str = None) -> bool:
        now = datetime.now().isoformat()
        ...
        sql = _adapt_sql(f"UPDATE schedules SET {set_clause} WHERE id=?")
        with managed_connection() as conn:
            cur = conn.cursor()
            cur.execute(sql, (*fields.values(), schedule_id))
            return cur.rowcount > 0

    @staticmethod
    def increment_retry(schedule_id: int) -> int:
        """Tăng retry_count +1, trả về giá trị mới."""
        with managed_connection() as conn:
            cur = conn.cursor()
            cur.execute(
    ...
    @staticmethod
    def cancel(schedule_id: int) -> bool:
        logger.info(f"[AUDIT] Huy lich dang ID={schedule_id}")
        return ScheduleModel.update_status(schedule_id, "cancelled")
