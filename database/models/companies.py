...
class CompanyModel:
    ...
    @staticmethod
    def get_by_id(company_id: int) -> dict:
    ...
    def get_by_workspace(workspace_id: int) -> dict:
        ...
        return CompanyModel.get_by_id(c_id)
    ...
    def upsert(workspace_id: int, **kwargs) -> int:
        ...
        if existing:
            CompanyModel.update(existing["id"], **kwargs)
            return existing["id"]
        else:
    ...
    def update(company_id: int, **kwargs) -> bool:
        ...
        sql = _adapt_sql(f"UPDATE companies SET {set_clause} WHERE id=?")
        with managed_connection() as conn:
            cur = conn.cursor()
            cur.execute(sql, (*fields.values(), company_id))
